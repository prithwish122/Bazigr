...
const U2U_BRIDGE_ADDRESS = "0xEA41526ac190C2521e046D98159eCCcC7a05F218"; // U2U bridge address
...
export function BridgeBox() {
  ...
  React.useEffect(() => {
    if (chainId) {
      if (chainId === 39) {
        // Currently on U2U, set as from network
        setFromNetwork("u2u")
  ...
  async function handleClaim(reward: number) {
    try {
      ...
      // Switch to U2U network for claiming
      ...
      const contract = new ethers.Contract(U2U_TOKEN_ADDRESS as `0x${string}`, bazigrAbi as any, signer)
  ...
  async function onBridge(fromAmount: string) {
    ...
    try {
      ...
      const needsNetworkSwitch = 
        (fromNetwork === "u2u" && currentChainId !== 39) ||
        (fromNetwork === "sepolia" && currentChainId !== 11155111)

      if (needsNetworkSwitch) {
        toast({ title: "Switching Network", description: `Switching to ${fromNetwork === "u2u" ? "U2U" : "Sepolia"} network...` })
      ...
      if (fromNetwork === "u2u" && toNetwork === "sepolia") {
        // Bridge from U2U to Sepolia
        await bridgeFromU2UToSepolia(amount)
      } else if (fromNetwork === "sepolia" && toNetwork === "u2u") {
  ...
  async function bridgeFromU2UToSepolia(amount: number) {
    ...
    const tokenContract = new ethers.Contract(U2U_TOKEN_ADDRESS, bazigrAbi, signer)
  ...
  async function bridgeFromSepoliaToU2U(amount: number) {
    ...
    toast({ title: "Step 3/4", description: "Switching to U2U network..." })
    ...
    toast({ title: "Step 4/4", description: "Unlocking tokens on U2U..." })
